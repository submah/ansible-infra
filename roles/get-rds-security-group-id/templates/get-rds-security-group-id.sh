...
/usr/bin/aws ec2 describe-security-groups --filter "Name=group-name,Values=prod-rds-security-group" --query 'SecurityGroups[*].[GroupId]' --output text > /tmp/get-rds-security-group-id.txt.out
